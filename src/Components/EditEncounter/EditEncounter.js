...
/**
 * Form for editing an encounter's details
 */
const EditEncounter = () => {
  ...
  const validateForm = () => {
    ...
    } else if (!visitCodePattern.test(encounterData.visitCode)) {
    ...
    if (!encounterData.billingCode.trim()) {
      ...
      if (!billingCodePattern.test(encounterData.billingCode)) {
    ...
    if (!encounterData.icd10.trim()) {
      ...
      if (!icd10Pattern.test(encounterData.icd10)) {
    ...
    if (!encounterData.date.trim()) {
      ...
      const providedDate = new Date(`${encounterData.date.trim()}T00:00:00Z`); // Set to midnight of the provided day
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (validateForm()) {
      fetch(`http://localhost:8085/patients/${patientId}/encounters/${encounterId}`, {
        method: 'PUT',
        headers: {
        ...
        .then((response) => {
          ...
          throw new Error('Failed to edit encounter');
        })
        .then(() => {
